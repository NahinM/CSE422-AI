...
class Population:
    def __init__(self, populationSize:int) -> None:
        self.members:list[Element] = [ Element([],0) for _ in range(populationSize)]
...
class AlgorithmRunner:
    def __init__(self, initialPopulationSize:int, maxPopulationSize:int, maxCrossoverLimit:int, crossoverPoint:int) -> None:
    ...
    def run(self,generationLimit:int, bestValue:int) -> None:
        Generation:int = 0
        population:Population = Population(self.InitialPopulationSize)
        population.fitness()
